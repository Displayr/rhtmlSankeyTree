...
#  TODO change data to use functional_test directory
  Scenario: Rerender Test
    Given I am viewing "data.functional_tests.minimal" with dimensions 500x500 and rerender controls
    Then the "minimal_500x500_base" snapshot matches the baseline
    When I rerender with config "data.functional_tests.minimal_split_50_50"
    And I wait for animations and state callbacks to complete
    Then the "minimal_split_50_50" snapshot matches the baseline
    And the final state callback should match "data.functional_tests.state_minimal.500x500_modified_data_50_50_split" within 0.5